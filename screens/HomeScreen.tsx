...
export default function HomeScreen() {
  ...
  useEffect(() => {
    ...
    let firstUploadBackgroundProcessTimer = setTimeout(async () => await uploadAllPhotos(), 30000);

    // this will clear Timeout
    // when component unmount like in willComponentUnmount
    // and show will not change to true

    //upload to the server every  hour
    const interval = setInterval(async () => {
      await uploadAllPhotos();
    }, 3600000);
    return () => {
      clearInterval(interval)
      clearTimeout(firstUploadBackgroundProcessTimer);
    };
  ...
  async function uploadAllPhotos() {
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      let details = await MediaLibrary.getAssetInfoAsync(data[i].id)
      console.log('result type', details.localUri)
    ...
    let result = await post(`/users/media-background`, options)
        .then((response) => {
          console.log('upload goes well', response)
        }).catch((error) => {
